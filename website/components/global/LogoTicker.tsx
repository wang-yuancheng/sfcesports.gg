...
export default function LogoTicker({ logoList }: { logoList: LogoItem[] }) {
  ...
  useEffect(() => {
    const recalc = () => {
      ...
      const copies = Math.ceil(Math.ceil(vw / minW) / logoList.length);
      setSets(copies);
    };
    recalc();
  ...
  const logos = useMemo(
    () => Array.from({ length: sets * 2 }, () => logoList).flat(),
    [sets, logoList]
  );

  return (
    <div className="section-container navbarsm:py-3 mb-5" ref={viewportRef}>
      <div className="w-full mt-0 sm:mt-0 mb-12">
